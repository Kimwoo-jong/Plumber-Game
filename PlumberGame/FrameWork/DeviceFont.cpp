...
bool g_DeviceFont::DrawString( const char* msg , int x , int y , D3DCOLOR color)
{
	...
	Fonts->DrawText( Sprite , msg , strlen( msg ) , &rect , DT_NOCLIP , D3DCOLOR_XRGB(255, 255,0) ) ;
	Sprite->End() ;
	
	return true;
}

bool g_DeviceFont::DrawStringColor(const char* msg, int x, int y, D3DCOLOR color)
{
	RECT rect = { x , y , fdesc.Width*strlen(msg) , fdesc.Height };
